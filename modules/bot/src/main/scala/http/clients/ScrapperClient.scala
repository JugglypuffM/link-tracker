...
object ScrapperClient {
  class BadRequestException extends Throwable with NoStackTrace

  class LinkNotFoundException extends Throwable with NoStackTrace
  ...
  final private class Impl(using config: AppConfig, client: SttpBackend[IO, Any], lm: Logging.Make[IO])
    ...
    def trackLink(id: Long, request: AddLinkRequest): IO[Unit] =
      for {
        _ <- info"Sending track request for $id"
        ...
        _ <- info"Got response for $id: ${response.toString}"
    ...
    def untrackLink(id: Long, request: RemoveLinkRequest): IO[Unit] =
      for {
        _ <- info"Sending untrack request for $id"
    ...
    def getLinkList(id: Long): IO[LinkListResponse] =
      for {
        _ <- info"Sending link list request for $id"
