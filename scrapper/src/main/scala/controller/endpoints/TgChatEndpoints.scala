...
object TgChatEndpoints {
  val registerEndpoint: Endpoint[Unit, Long, ApiErrorResponse, Unit, Any] =
    ...
    .errorOut(statusCode(StatusCode.BadRequest).description("Некорректные параметры запроса").and(jsonBody[ApiErrorResponse]))
  ...
  val deleteEndpoint: Endpoint[Unit, Long, ApiErrorResponse, Unit, Any] =
    endpoint.delete
      ...
      .errorOut(oneOf(
        oneOfVariant(statusCode(StatusCode.BadRequest).description("Некорректные параметры запроса").and(jsonBody[ApiErrorResponse])),
