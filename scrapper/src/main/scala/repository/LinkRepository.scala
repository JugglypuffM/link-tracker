...
trait LinkRepository[F[_]] {
  def save(chatId: Long, url: Uri): F[LinkInfo]
...
object LinkRepository {
  final private class InMemory(using repo: Ref[IO, Set[LinkInfo]], lastId: Ref[IO, Long]) extends LinkRepository[IO] {
    override def save(chatId: Long, url: Uri): IO[LinkInfo] =
      for {
        r <- repo.get
        _ <- if (r.exists(info => (chatId == info.chatId) && (url == info.link.url))) IO.raiseError(SaveError())
        else IO(())
        i <- lastId.get
        id = i + 1
        info <- r.find(url == _.link.url) match
          case Some(LinkInfo(link, _, _)) =>
            val info = LinkInfo(link, chatId, None)
          ...
          case None =>
            for {
              i <- lastId.get
              id   = i + 1
              info = LinkInfo(Link(id, url), chatId, None)
